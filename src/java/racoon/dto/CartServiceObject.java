 ...
 */
public class CartServiceObject implements Serializable {
    ...
    public CartServiceObject() {
        this.customerName = "Guest";
        cart = new HashMap<>();
    }

    public CartServiceObject(String customerName) {
    ...
    public List<ServiceDTO> getCartDetail() throws SQLException, ClassNotFoundException {
        this.total = 0;
        List<ServiceDTO> dtoList = new ArrayList<>();
        ServiceDAO dao = new ServiceDAO();
        for (String key : this.cart.keySet()) {//lấy toàn bộ id trong cart để tìm
            ServiceDTO dto = dao.findByPK(key);
