...
public class SearchRoomController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String ids = request.getParameter("txtSearch");
            RoomDAO dao = new RoomDAO();
            if (ids.length() == 0) {
                List<RoomDTO> list = dao.loadRoom();
                request.setAttribute("LIST", list);
            } else {
                int id = Integer.parseInt(ids);
                List<RoomDTO> dto = dao.findByNumber(id);
                request.setAttribute("LIST", dto);
            }

        } catch (Exception e) {
            log("Error at LoadRoomController " + e.getMessage());
        } finally {
